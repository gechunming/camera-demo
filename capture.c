...
#define V4L2_CID_CAMERA_RETURN_FOCUS		(V4L2_CID_PRIVATE_BASE + 127)
...
int fimc_v4l2_s_fmt_is(int fp, int width, int height, unsigned int fmt, enum v4l2_field field)
{
    ...
    v4l2_fmt.fmt.pix = pixfmt;
    printf("fimc_v4l2_s_fmt_is : width(%d) height(%d)", width, height);

    /* Set up for capture */
    if (ioctl(fp, VIDIOC_S_FMT, &v4l2_fmt) < 0) {
        printf("ERR(%s):VIDIOC_S_FMT failed", __func__);
        return -1;
    }
...
static int fimc_v4l2_querybuf_preview(int fp, enum v4l2_buf_type type, int nr_frames, int num_plane)
{
	struct v4l2_buffer v4l2_buf;
	...
	for (i = 0; i < nr_frames; i++) {
		...
		ret = ioctl(fp, VIDIOC_QUERYBUF, &v4l2_buf);
		if (ret < 0) {
			printf("ERR(%s):VIDIOC_QUERYBUF failed", __func__);
			return -1;
		}

		size = v4l2_buf.length;

		if ((addr_null = (char *)mmap(0, v4l2_buf.length, PROT_READ | PROT_WRITE, MAP_SHARED,
						fp, v4l2_buf.m.offset)) < 0) {
			printf("[%s %d] mmap() failed",__func__, __LINE__);
			return -1;
		}
		//printf("buffers[%d].virt.p = %p v4l2_buf.length = %d", i, addr_null, v4l2_buf.length);
	}
	return 0;
}
...
int camera_startPreview(void)
{
    ...
    //fimc_v4l2_s_fmt_is(m_cam_fd, m_preview_width, m_preview_height, m_preview_v4lformat, 0);
	...
	fimc_v4l2_querybuf_preview(m_cam_fd, V4L2_BUF_TYPE_VIDEO_CAPTURE, MAX_BUFFERS, 1);
...
int camera_setSnapshotCmd(void)
{
    ...
    //fimc_v4l2_s_fmt_is(m_cam_fd, m_snapshot_width, m_snapshot_height, V4L2_PIX_FMT_JPEG, 2);
...
int main()
{
	...
	printf("camera_startPreview ok\n");
	ret = camera_stopPreview();
	if (ret < 0) {
		return 0;
	}
	printf("camera_stopPreview ok\n");
