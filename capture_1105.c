...
static int read_frame_no_input(int fd, int record)
{
...
static int read_frame(int fd, int record)
{
    struct v4l2_buffer buf;
    unsigned int i;
	...
	int ret;
	ctrl.id = V4L2_CID_PRIVATE_BASE + 53;
	ctrl.value = 0;
	enum v4l2_buf_type type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
	if (record) {
		//pause 一下
		printf("--------------------------pause-----------and get jpeg file-------------\n");
		ret = ioctl(fd, VIDIOC_S_CTRL, &ctrl);
		if (ret) {
			printf("s_ctrl pause error !!!!!\n");
		}
...
static void mainloop_preview (int fd, int record, int looptime)
{
    unsigned int count;
    count = looptime;
...
int main()
{
   ...
   // lcd_open();
    ...
    fmt1.fmt.pix.pixelformat = V4L2_PIX_FMT_UYVY;
...
//    mainloop_preview(fd, 0, 10);

	//for new jpeg capture
	printf("preview stream off ..\n");
    type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
